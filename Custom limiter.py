...
def apply_styles(ws, threshold):
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        for cell in row:
            ...
            if not isinstance(cell.value, (int, float)): continue
            try:
...
def process_grid(data_df, cols, batch_subjects, low_thresh, high_thresh, show_all=False):
    ...
    # STRICT FILTERING: Ensure the row is included if ANY subject falls in the range
    in_range_mask = (full_grid[final_subjects] >= low_thresh) & (full_grid[final_subjects] <= high_thresh)
    
    if show_all:
        shortage_grid = full_grid.copy()
    else:
        shortage_grid = full_grid[in_range_mask.any(axis=1)].copy()
    
    if shortage_grid.empty: return None, None
    
    # Recalculate mask for the current grid to ensure counts match
...
if uploaded_file:
    ...
    with st.sidebar:
        ...
        with c_f: low_v = st.number_input("From (%)", 0.0, 100.0, 0.0, 0.01, format="%.2f")
        with c_t: high_v = st.number_input("To (%)", 0.0, 100.0, 75.0, 0.01, format="%.2f")
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        with tabs[0]:
            if summaries:
                ...
                st.plotly_chart(px.bar(sum_df, x='Section', y='Count', template="plotly_dark"), use_container_width=True)
                sum_df.to_excel(writer, sheet_name='SUMMARY', index=False)
            else: st.info("No students in range.")
